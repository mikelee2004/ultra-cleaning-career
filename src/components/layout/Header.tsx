...
const BASE = 'https://cleankrd23.ru';

// Ссылки ведут на основной сайт
const services = [
  { label: 'Генеральная уборка',      href: `${BASE}/generalcleaning` },
  { label: 'Поддерживающая уборка',   href: `${BASE}/regularcleaning` },
  { label: 'Уборка после праздника',  href: `${BASE}/postholidaycleaning` },
  { label: 'Уборка после ремонта',    href: `${BASE}/postrenovationcleaning` },
  { label: 'Химчистка мебели',        href: `${BASE}/furniturecleaning` },
  { label: 'Мытьё окон',              href: `${BASE}/windowcleaning` },
  { label: 'Коммерческая уборка',     href: `${BASE}/commercialcleaning` },
  { label: 'Уборка после пожара',     href: `${BASE}/firedamagecleaning` },
  { label: 'Озонирование помещений',  href: `${BASE}/ozonation` },
];
...
export default function Header() {
  ...
  return (
    <header className={cn(
      'fixed top-0 left-0 right-0 z-[1000] transition-all duration-300 backdrop-blur-md border-b',
      scrolled ? 'bg-white/98 shadow-[0_4px_20px_rgba(0,0,0,0.08)] border-transparent' : 'bg-white/95 border-black/5',
    )}>
      <div className="max-w-[1240px] mx-auto px-4">
        <div className="flex items-center justify-between h-16 md:h-[72px]">

          {/* Logo — ведёт на основной сайт */}
          ...
          <nav className="hidden lg:flex items-center gap-1" aria-label="Основная навигация">
            ...
            <a href={BASE} className="px-3 py-2 rounded-lg text-[#1C2B3A] font-semibold text-[15px] no-underline hover:text-[#A70000] transition-colors">
              Главная
            </a>
            <a href={`${BASE}/news`} className="px-3 py-2 rounded-lg text-[#1C2B3A] font-semibold text-[15px] no-underline hover:text-[#A70000] transition-colors">
              Новости
            </a>
          </nav>
          ...
          <div className="hidden lg:flex items-center gap-3">
            <a href={`tel:${contacts.phone}`} aria-label="Позвонить"
            ...
            <a href={`${BASE}/#calc`}
              className="px-[18px] py-[10px] rounded-xl bg-[#A70000] text-white font-bold text-sm whitespace-nowrap shadow-[0_4px_12px_rgba(220,38,38,0.2)] hover:-translate-y-px transition-transform no-underline">
              Рассчитать цену
            </a>
          </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-white border-t border-black/5 shadow-[0_20px_40px_rgba(0,0,0,0.1)] max-h-[calc(100vh-64px)] overflow-y-auto px-5 py-6">
          ...
          <div className="flex gap-3 mb-6">
            <a href={contacts.vk} target="_blank" rel="noopener noreferrer" aria-label="ВКонтакте"
              className="w-11 h-11 rounded-xl bg-[#0077FF] flex items-center justify-center text-white"><VkIcon size={22} /></a>
            <a href={contacts.whatsappWithMessage} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp"
              className="w-11 h-11 rounded-xl bg-[#25D366] flex items-center justify-center text-white"><WhatsAppIcon size={22} /></a>
            <a href={contacts.telegram} target="_blank" rel="noopener noreferrer" aria-label="Telegram"
              className="w-11 h-11 rounded-xl bg-[#229ED9] flex items-center justify-center text-white"><TelegramIcon size={22} /></a>
          </div>
          ...
          <div className="flex flex-col gap-3 mb-6">
            <a href={BASE} className="block px-4 py-[14px] rounded-xl border border-[#E2E8F0] text-[#1C2B3A] text-base font-semibold no-underline">Главная</a>
            <a href={`${BASE}/news`} className="block px-4 py-[14px] rounded-xl border border-[#E2E8F0] text-[#1C2B3A] text-base font-semibold no-underline">Новости</a>
          </div>
          <a href={`${BASE}/#calc`}
            className="block w-full py-4 rounded-xl bg-[#A70000] text-white font-bold text-base text-center no-underline shadow-[0_8px_20px_rgba(220,38,38,0.2)]">
...
function VkIcon({ size = 20 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor"><path d="M15.684 0H8.316C1.592 0 0 1.592 0 8.316v7.368C0 22.408 1.592 24 8.316 24h7.368C22.408 24 24 22.408 24 15.684V8.316C24 1.592 22.408 0 15.684 0zm3.692 17.123h-1.744c-.66 0-.864-.525-2.05-1.727-1.033-1-1.49-1.135-1.744-1.135-.356 0-.458.102-.458.593v1.575c0 .424-.135.678-1.253.678-1.846 0-3.896-1.118-5.335-3.202C4.624 10.857 4 8.408 4 7.932c0-.254.102-.491.593-.491h1.744c.44 0 .61.203.78.677.863 2.49 2.303 4.675 2.896 4.675.22 0 .322-.102.322-.66V9.721c-.068-1.186-.695-1.287-.695-1.71 0-.203.17-.407.44-.407h2.744c.373 0 .508.203.508.643v3.473c0 .372.17.508.271.508.22 0 .407-.136.813-.542 1.254-1.406 2.151-3.574 2.151-3.574.119-.254.322-.491.762-.491h1.744c.525 0 .643.27.525.643-.22 1.017-2.354 4.031-2.354 4.031-.186.305-.254.44 0 .78.186.254.796.779 1.203 1.253.745.847 1.32 1.558 1.473 2.05.17.491-.085.745-.576.745z"/></svg>;
}
